...
def dist2(x,y):
    return (torch.sum(torch.abs(x-y),dim=1)/x.size(1))
def dist3(x,y):
    return torch.linalg.norm(x-y,dim=1)/x.size(0)
def dist_inf(x,y):
    (v,_) = torch.max(torch.abs(x-y),dim=1,keepdim=True)
    return v/x.size(1)
...
#generator.load_state_dict(torch.load("generator_dist2.pt"))
#discriminator.load_state_dict(torch.load("discriminator_dist2.pt"))
batches_done = 0
n_epochs = 1000
batch_size = 1375
n_critic = 2
for epoch in range(n_epochs):
    ...
    if epoch % 200 ==1:
        ...
        mimic_set = generator(z).detach()
            #torch.save(generator.state_dict(), 'generator_dist2_700.pt')
            #torch.save(discriminator.state_dict(),'discriminator_dist2_700.pt')
#print("check before saving:",mimic_set)
        np.save('mimic_set_gen_'+str(epoch)+'_dist2c.npy',mimic_set.cpu().numpy())
